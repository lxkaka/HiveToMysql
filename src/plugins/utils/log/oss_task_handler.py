...
from plugins.hooks.oss_hook import OSSHook
...
class OSSTaskHandler(FileTaskHandler, LoggingMixin):
    ...
    def __init__(
        self, base_log_folder, oss_log_folder, filename_template, delete_local_copy
    ):
        super(OSSTaskHandler, self).__init__(base_log_folder, filename_template)
        self.remote_base = oss_log_folder
    ...
    def oss_log_exists(self, remote_log_location):
        ...
        """
        try:
            return self.hook.get_key(remote_log_location)
        except Exception:
            pass
        return False
    ...
    def oss_write(self, log, remote_log_location, append=True):
        ...
        if append and self.oss_log_exists(remote_log_location):
            old_log = self.oss_read(remote_log_location)
            log = "\n".join([old_log, log]) if old_log else log

        try:
            self.hook.load_string(
                string_data=log, key=remote_log_location, replace=True
            )
        except Exception:
